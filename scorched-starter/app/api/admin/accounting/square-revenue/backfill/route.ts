// app/api/admin/accounting/square-revenue/backfill/route.ts
// On-demand backfill for a date range, mirroring app/api/admin/shifts/sync's
// shape. Each day is independently idempotent (see revenue-job.ts), so
  ...
  try {
    ...
    for (const dateStr of datesBetween(startDate, endDate)) {
      for (const [squareLocationId, locationKey] of Object.entries(SQUARE_LOCATION_MAP)) {
        const key = `${locationKey}:${dateStr}`;
        try {
          results[key] = await postSquareRevenueForDay(squareLocationId, locationKey, dateStr);
        } catch (err) {
          console.error("SQUARE_REVENUE_BACKFILL_ERROR", key, err);
    ...
    console.error("SQUARE_REVENUE_BACKFILL_ERROR", err);
